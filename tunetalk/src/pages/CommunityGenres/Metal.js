...
function Metal() {
  ...
  const fetchFollowStatus = async () => {
    try {
      ...
      setIsFollowing(response.data.metal);
    } catch (err) {
  ...
  useEffect(() => {
    // Function to fetch Metal playlists using the access token
    const fetchMetalMusic = async () => {
      if (!accessToken) return;

      const response = await fetch('https://api.spotify.com/v1/browse/categories/metal/playlists', {
      ...
      const data = await response.json();
      setMetalPlaylists(data.playlists.items);
    };

    fetchMetalMusic().catch(error => {
      console.error('Fetching metal playlists failed:', error);
    });
  }, [accessToken]);

  useEffect(() => {
    const fetchFeaturedTrack = async () => {
      if (!accessToken || metalPlaylists.length === 0) return;

      const chosenPlaylist = metalPlaylists[0];
