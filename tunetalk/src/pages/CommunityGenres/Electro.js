...
function Electro() {
    ...
    const [ElectroPlaylists, setElectroPlaylists] = useState([]);
    const [user] = useUser(); 
    ...
    useEffect(() => {
      // Function to fetch Electro playlists using the access token
      const fetchElectroMusic = async () => {
        if (!accessToken) return;

        const response = await fetch('https://api.spotify.com/v1/browse/categories/Electro/playlists', {
        ...
        const data = await response.json();
        setElectroPlaylists(data.playlists.items);
      };

      fetchElectroMusic().catch(error => {
        console.error('Fetching Electro playlists failed:', error);
      });
    }, [accessToken]);
