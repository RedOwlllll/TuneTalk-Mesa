...
function Rap() {
  ...
  useEffect(() => {
    const fetchRapMusic = async () => {
      if (!accessToken) return;

      const response = await fetch('https://api.spotify.com/v1/browse/categories/Rap/playlists', {
      ...
      const data = await response.json();
      setRapPlaylists(data.playlists.items);
    ...
    fetchRapMusic().catch(error => {
      console.error('Fetching Rap playlists failed:', error);
    });
  }, [accessToken]);
