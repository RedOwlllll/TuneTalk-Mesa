...
function Electronic() {
  ...
  const fetchFollowStatus = async () => {
    try {
      ...
      setIsFollowing(response.data.electronic);
    } catch (err) {
  ...
  useEffect(() => {
    const fetchFeaturedTrack = async () => {
      if (!accessToken || electronicPlaylists.length === 0) return;

      const chosenPlaylist = electronicPlaylists[0];
  ...
  }, [accessToken, electronicPlaylists]);
  ...
  useEffect(() => {
    const communityName = 'electronic'; 
