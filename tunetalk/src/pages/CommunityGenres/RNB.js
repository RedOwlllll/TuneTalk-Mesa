...
function RNB() {
  ...
  //   if (!isFollowing) {
  //     const confirmed = window.confirm("By following the RNB community, song recommendations will show in your profile");
  //     if (confirmed) {
  //       try {
  //         if (!accessToken) {
  ...
  //             }));
  //             setRandomTrack(randomTrack); // Store the randomTrack
  //           }
  //         }
  //       } catch (err) {
  ...
  //     }
  //   } else {
  //     const confirmed = window.confirm("By unfollowing the RNB paage, songs recommended will be removed");
  //     if (confirmed) {
  //       try {
  ...
  //         // Clear all recommendations from the user's state
  //         setUser(prevUser => ({
  //           ...prevUser,
  //           recommendations: [] // Clear all recommendations
  //         }));
  //         setRandomTrack(null); // Clear the randomTrack
  //       } catch (err) {
    ...
    if (!isFollowing) {
        ...
        try {
          ...
          if (playlists.length > 0) {
            ...
            if (tracksData.items.length > 0) {
              ...
              // Store the random track ID in localStorage
              localStorage.setItem('randomTrackId', randomTrack.id);
            }
          }
        } catch (err) {
          console.error("Error updating follow status:", err);
      ...
      if (confirmed) {
        try {
          ...
          // Clear the random track ID from localStorage
          localStorage.removeItem('randomTrackId');
        } catch (err) {
          console.error("Error updating un-follow status:", err);
        }
      }
  ...
  const fetchFollowStatus = async () => {
    try {
      ...
      setIsFollowing(response.data.rnb);
    } catch (err) {
  ...
  useEffect(() => {
    const fetchFeaturedTrack = async () => {
      if (!accessToken || rnbPlaylists.length === 0) return;

      const chosenPlaylist = rnbPlaylists[0];
