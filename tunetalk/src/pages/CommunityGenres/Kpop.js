...
function Kpop() {
  ...
  const fetchFollowStatus = async () => {
    try {
      ...
      setIsFollowing(response.data.kpop);
    } catch (err) {
  ...
  useEffect(() => {
    const fetchFeaturedTrack = async () => {
      if (!accessToken || kpopPlaylists.length === 0) return;

      const chosenPlaylist = kpopPlaylists[0];
