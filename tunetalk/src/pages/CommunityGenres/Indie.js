...
function Indie() {
  ...
  const fetchFollowStatus = async () => {
    try {
      ...
      setIsFollowing(response.data.indie);
    } catch (err) {
  ...
  useEffect(() => {
    const fetchFeaturedTrack = async () => {
      if (!accessToken || indiePlaylists.length === 0) return;

      const chosenPlaylist = indiePlaylists[0];
